...
import org.sdrc.cpmisweb.service.CciSHService;
...
/**
 * @author Pratyush(pratyush@sdrc.co.in) created on : 08-Mar-2018 5:38:33 pm
 */
@Controller
public class CciSHController {

	@Autowired
	private StateManager stateManager;
	@Autowired
	CciSHService cciHomeService;
	@Autowired
	private UserService userService;
	
	@Authorize(feature="data_entry",permission="edit")
	@RequestMapping(value =PathConstants.CCI_SPECIAL_HOME_CONTROLLER_SAVE, method = RequestMethod.POST)
	@ResponseBody
	public ResponseEntity<Object> saveCciHome(@RequestBody CciSHModel cciHomeModel) {
	...
	@Authorize(feature="data_entry",permission="edit")
	@RequestMapping(value =PathConstants.AUTO_SAVE_CCISH, method = RequestMethod.POST)
	@ResponseBody
	public ResponseEntity<Object> autoSaveCciSplHomeMethod(@RequestBody CciSHModel cciHomeModel,HttpServletRequest request, HttpServletResponse resp) throws ServletException{
	...
	@Authorize(feature="data_entry",permission="edit")
	@RequestMapping(value = PathConstants.CCI_SPECIAL_HOME_CONTROLLER_SUBMIT, method = RequestMethod.POST)
	@ResponseBody
	public ResponseEntity<Object> submitCciHome(@RequestBody CciSHModel cciHomeModel) {
		if(QuaterlyAggreationJob.isAggregationGoingOn || MonthlyAggregationJob.isAggregationGoingOn){
			return new ResponseEntity<Object>(HttpStatus.TEMPORARY_REDIRECT);
		}else{
	...
	@RequestMapping(value =PathConstants.CCI_SPECIAL_HOME_CONTROLLER_GET_DATA, method = RequestMethod.POST)
	@ResponseBody
	public CciSHModel getLastSavedCciHomeData(@RequestBody Integer userId) {
