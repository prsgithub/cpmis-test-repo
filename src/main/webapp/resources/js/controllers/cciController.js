cpmisApp.controller('CCIController', function($scope, $http, $timeout, $window, commonService) {
	...
	$scope.checkD = function() {
		...
		$scope.messageField2="C.1.Total children in the month";
	...
	$scope.checkF = function() {
		...
		$scope.messageField2="E.1.Total children on the last day the month";
	...
	$scope.checkFMinority = function() {
		...
		$scope.messageField2="F.1.Total number of children";
		$scope.alertMessage = "value can not exceed";
		return commonFunctionBody(firstInput, secondInput, bgc);
	};
	...
	$scope.checkK11n12 = function() {
		...
		$scope.messageField2="E. total children on the last day of the reporting month( value= "+firstInput+")";
		$scope.alertMessage = "value should be equal to";
	...
	$scope.checkJNegetive=function(currentId,targetId,msg){
		...
		$scope.alertMessage = "can not be less than";		
		if (firstInput < secondInput) {
			bgc.style.setProperty("background-color", "#DC143C", "important");
			bgc.style.setProperty("color", "#FFFAF0", "important");
			bgc.style.setProperty("border-color", "red", "important");
	...
	$scope.checkJNegetive1=function(currentId,j3Id,targetId,msg){
	 ...
	 $("input[type='number']").keyup(function() {
		 checkD();checkF();checkFMinority();checkG();checkH5();checkI5();checkJ1();checkJ2();checkJ3();checkJ4();removeCheckK11n12();removeNegetiveColor();  
	});
	...
	$scope.calculateWithC = function(id, modelId, yearRange) {
		...
		if (yearRange == '7-11') {
			...
			$scope.messageField2="C.1.Total children in the month (7-11)years";
			$scope.alertMessage = "value can not exceed";
			...
			$scope.messageField2="C.1.Total children in the month (12-15)years";
			$scope.alertMessage = "value can not exceed";
			...
			$scope.messageField2="C.1.Total children in the month (16-18)years";
			$scope.alertMessage = "value can not exceed";
			...
			$scope.messageField2="C.1.Total children in the month (>18)years";
			$scope.alertMessage = "value can not exceed";
	...
	$scope.saveOH = function(cciForm, clickType, submitConfirm) {
		...
		if (clickType == 'save') {
				prepareObj(cciForm);
				$http.post('saveCciObservationHome', cciForm).then(function(result) {
					commonService.checkSessionOut(result);
					if (result.status == 200) {
						$(".loader").fadeOut();
		...
		if (submitConfirm == 'ok') {
			prepareObj(cciForm);
			$http.post('submitCciObservationHome', cciForm).then(function(result) {
				commonService.checkSessionOut(result);
				if (result.status == 200) {
	...
	$scope.autoSaveOH = function(cciForm, clickType) {
		cciForm.homeType = 'OH';
		...
		if (clickType == 'save') {
				prepareObj(cciForm);
				$http.post('autoSaveCciOH', cciForm).then(function(result) {
					commonService.checkSessionOut(result);
					if (result.status == 200) {
						$(".loader").fadeOut();
						$('#successModalForAutoSaveOH').modal('show');
					}
				}, function(error) {
	...
	$scope.timerForSave=function ($scope,$timeout) {
	    ...
	    $scope.onTimeout = function(){
	        ...
	        if($scope.counterSeconds==0 && !$('#confirmationModalForCCCI').hasClass('in')){
	        	$scope.stop();
	        	$scope.autoSaveOH($scope.cciForm, 'save');
	        return false;
	        }
	...
	var lastDay;
    var firstDay;
	$http.post("getLastSavedCciObservationHomeData", currentUserId).then(function(result) {
		if (result.data != "")
	...
	$scope.resetAccordionOthers = function(cciTrainingDetails) {
		...
		else if (cciTrainingDetails.j1eTopicTraining.typeDetailsId != 10)
			cciTrainingDetails.totOthersSpecify = null;
	};
